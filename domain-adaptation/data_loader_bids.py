...
class BidsMRI(data.Dataset):
    ...
    def __init__(self, path, labels_path, target=None, encode_target=False, load_online=False, 
        ...
        for patient_folder_name in os.listdir(self.path_to_folder):
            if 'sub-' in patient_folder_name and os.path.isdir(self.path_to_folder + patient_folder_name) and \
                self.type in os.listdir(self.path_to_folder + patient_folder_name):        
                temp_path = self.path_to_folder + patient_folder_name + "/" + self.type + "/"
